...
public class CardService implements ICardService {
    ...
    @Override
    @Transactional
    public Mono<CardDto> createCard(Card card) {
        log.info("save card >>>");
        return cardRepository.findByProductId(card.getProductId())
                ...
                .flatMap(card1 -> cardHelper.setObjectCardBySave(card))
    ...
    @Override
    @Transactional
    public Mono<CardDto> updateCard(Card card, String id) {
        log.info("update card >>>");
        return cardRepository.findById(id)
                .switchIfEmpty(Mono.just(new Card()))
                .flatMap(findCard -> cardRepository.findByProductId(findCard.getProductId())
                        .flatMap(card1 -> cardHelper.setObjectCardByUpdate(card, findCard))
    ...
    public Mono<CardDto> removeCard(String id) {
        ...
        return cardRepository.findById(id)
                ...
                .flatMap(card -> webClientProductHelper.deleteProduct(card.getProductId())
                        .flatMap(isBool -> isBool ? Mono.just(new CardDto()) : Mono.empty()));
